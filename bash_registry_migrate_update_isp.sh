...
NEW_REG_IP="$2"
# Other variables
TARGET_DIR="/home/bchambers/reg_migration"
...
while IFS= read -r domain; do
	echo "$(date '+%F %T'): Creating new tables and view for '$domain'."| tee -a "$LOGFILE"
	if registry_create_tables; then
		...
		echo "$(date '+%F %T'): Table creation failed for '$domain'. Logging failed domain and proceeding to next domain"| tee -a "$LOGFILE" "$FAILED_DOMAINS"
		continue
	fi
